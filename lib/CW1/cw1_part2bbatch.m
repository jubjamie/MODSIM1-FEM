...
Batch=problemBatch(@cw1_part2b,[0.5 294.15 10],[1.5 322.15 10],[5 5 1]);
...
ylabel('TL (K)');
zlabel('c value');
heldXpos=((heldXindex-1)/Batch{1}.mesh.ne)*...
    (Batch{1}.mesh.xmax-Batch{1}.mesh.xmin);
title(['Variation in Temperature at x=' num2str(round(heldXpos,3))...
    'm with Q and TL']);
colorbar;
...
f11=figure('pos',[0,400,820,475],'Name',...
    'Temperature Profile - Part 2b - Varying TL');
holdPindex=4;
...
zlabel('c value');
holdPterm=1;
holdPvalue=(((Batch{1}.BatchOptions.UB(holdPterm)-...
    Batch{1}.BatchOptions.LB(holdPterm))/...
    (Batch{1}.BatchOptions.STEPS(holdPterm)-1))*(holdPindex-1))+...
    Batch{1}.BatchOptions.LB(holdPterm);
...
f12=figure('pos',[400,0,820,475],'Name',...
    'Temperature Profile - Part 2b - Varying Q');
holdPindex=1;
...
% Compare to part2a.
Batch2a=problemBatch(@cw1_part2a,[0.5 294.15 10],[1.5 322.15 10],[5 5 1]);
...
holdPvalue=(((Batch{1}.BatchOptions.UB(holdPterm)-...
    ...
    (Batch{1}.BatchOptions.STEPS(holdPterm)-1))*(holdPindex-1))+...
    Batch{1}.BatchOptions.LB(holdPterm);
title(['Difference between Part 2a and Part 2b as TL Only Varies. Q='...
    num2str(holdPvalue)]);
colorbar;
...
[x,y,z2b]= generateContormatrices(Batch,1,2,2);
[~,~,z2a]= generateContormatrices(Batch2a,1,2,2);
z=z2b-z2a;
