...
PlotOpts.title='Part 2a/b Parameter Extremities Comparison';
PlotOpts.filepath='status/part2b_comp_mini.png';
PlotOpts.x.label='x (m)';
PlotOpts.y.label='Temperature (K)';
PlotOpts.legend.labels=[strcat('Part a:',...
    getBatchTitles(Batcha)) strcat('Part b:',getBatchTitles(Batchb))];
PlotOpts.nodePlot.Color={'r','g','b',[1, 0.666, 0],'r','g','b',[1, 0.666, 0]}';
PlotOpts.nodePlot.LineStyle={'--','--','--','--','-','-','-','-'};
extraFcn=['grid;set(gca,''fontsize'',12);'...
