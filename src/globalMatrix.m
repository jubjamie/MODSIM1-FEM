...
%           Problem.Diffusion.LE.Generator: A function handle for a function returning the
%                                           local element matrix. Optional, Default will be set.
%           Problem.Diffusion.LE.coef: The diffusion coefficient D.
%           Problem.Reaction.LE.Generator: A function handle for a function returning the
...
%           Problem.f.fcn: A function handle for a run-time integrated & compiled function for 
%                          polynomial source terms. Definition example:
%                          Problem.f.fcn=sourceVector('1+(4*x)'); where x is the term for integration.
%           Problem.BCS.N: An array of Neumann boundary conditions using value@x notation. 
%                          e.g.[[2,0];,[0,1];]; Sets gradient of 2 at x=0 and gradient of 0 at x=1.
%           Problem.BCS.D: An array of Dirichlet boundary conditions using value@x notation. 
%                          e.g.[[2,0];,[0,1];]; Sets value of 2 at x=0 and value of 0 at x=1.
%   Outputs
...
% If no local elem diffusion generator function is defined then set the default from Part 1a.
if(~isfield(Problem,'Diffusion') || ~isfield(Problem.Diffusion,'LE') || ~isfield(Problem.Diffusion.LE,'Generator'))
...
if(~isfield(Problem,'Diffusion') || ~isfield(Problem.Diffusion,'LE') || ~isfield(Problem.Diffusion.LE,'coef'))
    Problem.Diffusion.LE.Generator=@(a,b,c) 0; % Set the local elem function to empty anon fcuntion.
    Problem.Diffusion.LE.coef=0;
end

% If no local elem reaction generator function is defined then set the default from Part 1b.
...
for i=1:N-1 % Loop through each element, creating its entry into the global matrix/source vector.
    M(i:i+1,i:i+1)=M(i:i+1,i:i+1)+... % Add the previous value to the new ones. (Allows local elem overlap)
    ...
    %^^ Insert local element matricies for diffusion and reaction as specified by the generator function.
    %^^ By using function handles referring to a local element function, this code can be extended to
    %^^ other problems with other function for generating local element matricies easily.
    ...
    % Populate the source vector by multiplying constant terms by a polynomial function of x0,1 and the Jacobian.
    f(i:i+1,1)=f(i:i+1,1)+(Problem.f.coef*Problem.f.fcn(x0,x1)*Problem.mesh.elem(i).J);
    %^^ By using this function handle system, the appropiate source vector generator can be compiled at runtime
    %^^ to the specified polynomial without the user having to integrate the variable source term.
end

%-----------%
% Enforce Neumann Boundaries
% Check if any Neumann BCs are specified and are of the correct size. (i.e. [dc/dx, x-position].
if(isfield(Problem,'BCS') && isfield(Problem.BCS,'N') && size(Problem.BCS.N,1)>0 && size(Problem.BCS.N,2)==2)
    for j=1:size(Problem.BCS.N,1) % Loop through Neumann BCs.
        ...
        assert(isequal(BCn(2),Problem.mesh.xmin) || isequal(BCn(2),Problem.mesh.xmax),...
            'N.Boundary condition not specified for an end node');
        
        equivRow=((BCn(2)/(Problem.mesh.xmax-Problem.mesh.xmin))*(N-1))+1; % Corresponding BC Vector node.
        BCrhs(equivRow)=BCn(1)*((2*BCn(2))-1); % Apply Neumann boundary at node in BC vector.
    end
end

%-------------%
% Enforce Dirichlet BCs
% Check if any Dirichlet BCs are specified and are of the correct size. (i.e. [c, x-position].
if(isfield(Problem,'BCS') && isfield(Problem.BCS,'D') && size(Problem.BCS.D,1)>0 && size(Problem.BCS.D,2)==2)
    for j=1:size(Problem.BCS.D,1) % Loop through the Dirichlet BCs.
        ...
        equivRow=((BCd(2)/(Problem.mesh.xmax-Problem.mesh.xmin))*(N-1))+1; % Corresponding BC Vector node.
        % Assert that the x-position represents a node/row number in the global matrix
