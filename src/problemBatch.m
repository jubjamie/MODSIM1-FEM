    ...
    for i=1:paramSize(1)
        ...
        for j=1:paramSize(2)
            ...
            for k=1:paramSize(3)
                ...
                disp(['Making Problem with params>> Arg1:' num2str(term1) ' Arg2:'...
                    num2str(term2) ' Arg3:' num2str(term3)]);
                if k==paramSize(3)
                ...
                Batch{posCounter}=problemTemplate(term1,term2,term3); % Create problem with parameters
                Batch{posCounter}.initParams=[term1 term2 term3]; % Save the paramteres used.
                Batch{posCounter}.BatchOptions.template=problemTemplate; % Save the template
                ...
                Batch{posCounter}.BatchOptions.BatchSize=batchSize; % Save the Batch size.
                totalNodes=totalNodes+Batch{posCounter}.mesh.ngn; % Cumulative global nodes in Batch
    ...
    % Loop through first parameter
        for j=1:paramSize(1)
            ...
            for k=1:paramSize(2)
                ...
                % Save Problem & Batch info to an element in the Batch.
                Batch{posCounter}=problemTemplate(term1,term2); % Create problem with parameters
                Batch{posCounter}.initParams=[term1 term2]; % Save the paramteres used.
                Batch{posCounter}.BatchOptions.template=problemTemplate; % Save the template
                % Save the bounds/step info
                Batch{posCounter}.BatchOptions.LB=LB;
                Batch{posCounter}.BatchOptions.UB=UB;
...
elseif size(LB,2)==1
    ...
    for k=1:paramSize(1)
        ...
        % Save Problem & Batch info to an element in the Batch.
        Batch{posCounter}=problemTemplate(term1); % Create problem with parameters
        Batch{posCounter}.initParams=[term1]; % Save the paramteres used.
        Batch{posCounter}.BatchOptions.template=problemTemplate; % Save the template
        % Save the bounds/step info
        Batch{posCounter}.BatchOptions.LB=LB;
        Batch{posCounter}.BatchOptions.UB=UB;
